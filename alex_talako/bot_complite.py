...
async def run_all_tests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await execute_command(
        "pytest -s -v class_work/class_work_10/test --alluredir=./allure-results",
        update
    )
...
async def run_ui_tests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Запуск pytest
    result = await execute_command(
        "pytest -s -v class_work/class_work_10/test/ui/ --alluredir=./allure-results",
        update
    )
...
async def run_api_tests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Запуск pytest
    result = await execute_command(
        "pytest -s -v class_work/class_work_10/test/api/ --alluredir=./allure-results",
        update
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        'Хей, я бот дипломного проекта для начинающего тестировщика!\n\nВот, что я могу тебе предложить:\n\n'
