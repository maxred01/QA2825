import os
from anna_voytovich.class_work.class_work_12.page.base_page import WebPage
from anna_voytovich.class_work.class_work_12.page.elements import WebElement
from anna_voytovich.class_work.class_work_12.page.elements import ManyWebElements


class AboutPage(WebPage):
    def __init__(self, web_driver, url=''):
        if not url:
            url = os.getenv('ABOUT_PAGE') or 'http://www.scania-minsk.by/about'

        super().__init__(web_driver, url)

    text_name_about = WebElement(id='//h1[@id="i54rqswgn_0"]')
    side_bar_truck = WebElement(id='(//span[contains(concat(' ', normalize-space(@class), ' '),  "text-block-wrap-div" )])[45]')
    side_bar_spares = WebElement(id='((//*[@data-do-section="{"screen":{"scroll":false,"smooth":true}}"])[0]//span)[6]')
